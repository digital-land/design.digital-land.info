...
OrgMapController.prototype.setup = function () {
  ...
  this.clickMarker = document.querySelector('.click-marker')
...
OrgMapController.prototype.clickHandler = function (e) {
  ...
  // sometimes the same feature appears multiple times in the list so need to filter
  console.log("Unique features:", this.removeDuplicates(features))
  ...
  var popup = new maplibregl.Popup({ maxWidth: this.popupWidth }).setLngLat(coordinates).setHTML(popupHTML).addTo(map)
}
